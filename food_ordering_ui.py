...
def print_check(my_order):
    ...
    print("\nYour Order:")
    total = 0
    for item_code, quantity in my_order:
...
def change_order(my_order):
    ...
    while True:
        print("\nCurrent Order:")
        for index, (item_code, quantity) in enumerate(my_order, start=1):
        ...
        if choice == 'C':
            ...
            if 0 <= item_num < len(my_order):
                my_order[item_num] = (my_order[item_num][0], new_quantity)
                print(f"Updated {my_order[item_num][0]} to quantity {new_quantity}.")
            else:
            ...
            if 0 <= item_num < len(my_order):
                removed_item = my_order.pop(item_num)
